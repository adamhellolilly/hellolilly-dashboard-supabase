// api/backfill.js
// One-time backfill — call once to populate last 90 days of Facebook + TikTok data
...
module.exports = async function handler(req, res) {
  ...
  const preset   = days <= 30 ? 'last_30d' : 'maximum';
  ...
  // ── FACEBOOK BACKFILL ───────────────────────────────────────────────────
  try {
    const fields = 'campaign_id,campaign_name,spend,impressions,reach,frequency,cpm,clicks,ctr,cpc,date_start';
    ...
    const campRows = (campData.data || []).map(d => ({
      ...
      reach: parseInt(d.reach || 0), clicks: parseInt(d.clicks || 0), leads: 0,
  ...
  try {
    ...
    while (chunkEnd > startDateObj) {
      ...
      const queryParams = [
        ...
        `metrics=${encodeURIComponent(JSON.stringify(['campaign_name','spend','impressions','reach','clicks','ctr','cpc','cpm','frequency','result','cost_per_result']))}`,
    ...
    if (allRows.length) {
      await supabaseUpsert('campaign_snapshots', allRows, 'date,platform,campaign_id');
    }
