  ...
  try {
    ...
    while (chunkEnd > startDateObj) {
      ...
      const queryParams = [
        ...
        `metrics=${encodeURIComponent(JSON.stringify(['campaign_name','spend','impressions','reach','clicks','ctr','cpc','cpm','frequency','on_site_form','cost_per_on_site_form']))}`,
      ...
      const chunkRows = (insData.data?.list || []).map(r => ({
        ...
        leads:         parseInt(r.metrics?.on_site_form || 0),
        cpm:           parseFloat(r.metrics?.cpm || 0),
        ctr:           parseFloat(r.metrics?.ctr || 0),
        cpc:           parseFloat(r.metrics?.cpc || 0),
        cost_per_lead: parseFloat(r.metrics?.cost_per_on_site_form || 0) || null,
