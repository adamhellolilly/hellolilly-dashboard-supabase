module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // TikTok leads from HubSpot UTM
    if (funnel && byPlatform['tiktok']) {
      byPlatform['tiktok'].leads = funnel.tiktok_leads_month || 0;
    }

    // Pick correct period KollaROM and customers
    let kollarom, customers;
    if (days <= 7) {
      kollarom  = funnel?.kollarom_30d  || 0; // use 30d as proxy for 7d
      customers = funnel?.customers_30d || 0;
    } else if (days <= 30) {
      kollarom  = funnel?.kollarom_30d  || 0;
      customers = funnel?.customers_30d || 0;
    } else if (days <= 60) {
      kollarom  = funnel?.kollarom_60d  || 0;
      customers = funnel?.customers_60d || 0;
    } else if (days <= 90) {
      kollarom  = funnel?.kollarom_90d  || 0;
      customers = funnel?.customers_90d || 0;
    } else {
      kollarom  = funnel?.kollarom_total  || 0;
      customers = funnel?.customers_total || 0;
    }
