...
module.exports = async function handler(req, res) {
  ...
  try {
    const fields = 'campaign_id,campaign_name,spend,impressions,reach,frequency,cpm,clicks,ctr,cpc';
    const filter = JSON.stringify(ROM_CAMPS);

    // Campaign level
    const campRes = await fetch(
      `${FB_BASE}/${account}/insights?fields=${fields}&date_preset=today&level=campaign&filtering=[{"field":"campaign.id","operator":"IN","values":${filter}}]&limit=100&access_token=${token}`
    );
    ...
    const adRes = await fetch(
      `${FB_BASE}/${account}/insights?fields=${fields},ad_id,ad_name&date_preset=today&level=ad&filtering=[{"field":"campaign.id","operator":"IN","values":${filter}}]&limit=200&access_token=${token}`
    );
