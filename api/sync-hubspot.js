...
module.exports = async function handler(req, res) {
  ...
  const last30Ts   = new Date(Date.now() - 30 * 86400000).getTime();
  ...
  try {
    // All-time counts
    const newKolla    = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'}]}]);
    const newCust     = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'}]}]);
    const oldKolla    = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}]);
    const oldCust     = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}]);

    // Last 30 days — KollaROM entered in last 30 days
    const newKolla30  = await count([{filters:[
      {operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_736277986',value:String(last30Ts)},
    ]}]);
    const newCust30   = await count([{filters:[
      {operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_customer',value:String(last30Ts)},
    ]}]);
    const oldKolla30  = await count([{filters:[
      {operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_736277986',value:String(last30Ts)},
    ]}]);
    const oldCust30   = await count([{filters:[
      {operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_customer',value:String(last30Ts)},
    ]}]);

    // Superheta this month
    ...
    // TikTok KollaROM all-time and last 30 days
    const ttKolla     = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'Smartcampaign_ROM'}]}]);
    const ttCust      = await count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'Smartcampaign_ROM'}]}]);
    const ttKolla30   = await count([{filters:[
      {operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'Smartcampaign_ROM'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_736277986',value:String(last30Ts)},
    ]}]);
    const ttCust30    = await count([{filters:[
      {operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'Smartcampaign_ROM'},
      {operator:'GTE',propertyName:'hs_v2_date_entered_customer',value:String(last30Ts)},
    ]}]);
    ...
    const row = {
      ...
      kollarom_30d: newKolla30 + oldKolla30 + ttKolla30,
      customers_30d: newCust30 + oldCust30 + ttCust30,
    };
