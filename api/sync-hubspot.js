...
module.exports = async function handler(req, res) {
  ...
  async function countWithRetry(filterGroups, retries=3) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (d.status === 'error' && d.message?.includes('limit')) {
          await sleep(1100);
          continue;
        }
  ...
  async function countSequential(queries) {
    const results = [];
    for (const q of queries) {
      results.push(await countWithRetry(q));
      await sleep(300);
    }
    return results;
  }
  ...
  try {
    const [
      ...
      [{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}],
      [{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}],
