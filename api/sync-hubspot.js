...
module.exports = async function handler(req, res) {
  ...
  const ts7d     = new Date(Date.now() -  7 * 86400000).getTime();
  const ts30d    = new Date(Date.now() - 30 * 86400000).getTime();
  const ts60d    = new Date(Date.now() - 60 * 86400000).getTime();
  const ts90d    = new Date(Date.now() - 90 * 86400000).getTime();
  ...
  function custFilter(sourceFilters, ts) {
  ...
  try {
    ...
    const row = {
      ...
      kollarom_60d:  newKolla60 + oldKolla60 + ttKolla60,
