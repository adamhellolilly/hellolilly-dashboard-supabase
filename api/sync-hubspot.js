...
module.exports = async function handler(req, res) {
  ...
  try {
    const [
      ...
      oldKolla, oldCust,
      superheta,
      // TikTok leads — contacts whose first URL contains TikTok ROM UTM
      ttLeadsMonth,
      ttKolla,
      ttCust,
    ] = await Promise.all([
      // Facebook new campaign KollaROM + customers
      count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'}]}]),
      count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'EQ',propertyName:'hs_latest_source_data_2',value:'huvudkampanjapi'}]}]),
      // Facebook old campaign KollaROM + customers
      count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}]),
      count([{filters:[{operator:'EQ',propertyName:'lifecyclestage',value:'customer'},{operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'120232042406430116'}]}]),
      ...
      // TikTok ROM leads this month (via utm_campaign in first URL)
      count([{filters:[
      ...
      // TikTok ROM KollaROM (all time)
      count([{filters:[
        {operator:'EQ',propertyName:'lifecyclestage',value:'736277986'},
        {operator:'CONTAINS_TOKEN',propertyName:'hs_analytics_first_url',value:'Smartcampaign_ROM'},
      ]}]),
      // TikTok ROM customers (all time)
      count([{filters:[
