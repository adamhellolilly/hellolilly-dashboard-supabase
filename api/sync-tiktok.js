...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const queryParams = [
      ...
      `metrics=${encodeURIComponent(JSON.stringify(['campaign_name','spend','impressions','reach','clicks','ctr','cpc','cpm','frequency','conversion','cost_per_conversion']))}`,
      `start_date=${today}`,
      `end_date=${today}`,
      `page_size=100`,
    ].join('&');
    ...
    const rows = (insData.data?.list || []).map(r => ({
      ...
      leads: parseInt(r.metrics?.conversion || 0),
      cpm:           parseFloat(r.metrics?.cpm || 0),
      ctr:           parseFloat(r.metrics?.ctr || 0),
      cpc:           parseFloat(r.metrics?.cpc || 0),
      cost_per_lead: parseFloat(r.metrics?.cost_per_conversion || 0) || null,
