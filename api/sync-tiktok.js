...
module.exports = async function handler(req, res) {
  ...
  try {
    const params = new URLSearchParams({
      ...
      metrics: JSON.stringify(['campaign_name','spend','impressions','reach','clicks','ctr','cpc','cpm','frequency','result','cost_per_result']),
      start_date: today,
      end_date: today,
      page_size: 100,
    });

    const insRes = await fetch(`${TT_BASE}/report/integrated/get?${params.toString()}`, {
      method: 'GET',
